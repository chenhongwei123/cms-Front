...
define(["jquery", "cookie", "sweetalert", "jqueryTime", "bootstrap"], function($) {
	...
	$(".add-activity").on("click", function() {
		if($("#extra").val() === "") {
			...
			$.ajax({
				type: "get",
				//url: "http://bjpsc.honganjk.com/admin/addBrand.json",
				url:urlb("admin/addBrand.json"),
				async: true,
				headers: {
					"code": $.cookie("code"),
					"token": $.cookie("token")
			...
			$(".a2").on("click", function(e) {
				...
				if($(this).parents('tr').children("td").eq(1).children('input').eq(0).val() == '') {
					...
					$.ajax({
						...
						url:urlb("admin/handleBrand.json"),
						headers: {
							"code": $.cookie("code"),
							"token": $.cookie("token")
						},
